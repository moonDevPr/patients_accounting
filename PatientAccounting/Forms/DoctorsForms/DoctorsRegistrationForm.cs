...
{
    public partial class DoctorsRegistrationForm : Form
    {
        private bool passCaptcha = false;

        public DoctorsRegistrationForm()
        {
            InitializeComponent();
        }
        ...
        private void CreateAccountButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Фамилия врача не может содержать числовые значения");
                SurnameBox.Focus();
            ...
            {
                MessageBox.Show("Имя врача не может содержать числовые значения");
                NameBox.Focus();
                NameBox.SelectAll();
                return;
            }
            ...
            {
                MessageBox.Show("Отчество врача не может содержать числовые значения");
                PatronymicBox.Focus();
                PatronymicBox.SelectAll();
                return;
            }

            var doctor = CreateDoctorRecord();

            var userCredential = CreateUserCredentials(doctor.id);

            SaveToDatabase(doctor, userCredential);

            MessageBox.Show("Учетная запись врача создана, для дальнейшего использования авторизуйтесь");
        ...
        private Doctors CreateDoctorRecord()
        {
            return new Doctors
            {
        ...
        private UsersCredentials CreateUserCredentials(int DoctorId)
        {
        ...
        private void SaveToDatabase(Doctors doctor, UsersCredentials userCredential)
        {
            using (var context = new ApplicationDbContext())
            {
                using (var transaction = context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        doctor.id_user_credential = userCredential.id;
        ...
        private static UsersRole? getRoleId()
        {
            ...
            using (var context = new ApplicationDbContext())
            {
                role = context.UsersRoles
                    .FirstOrDefault(elem => elem.role_name == "Врач");
                if (role != null)
                {
                    return role;
                }
